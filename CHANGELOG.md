
# 2.2.0

* 【操作习惯变更】免登录
* 【重构】重构 AI 调用模块，使用 ToolCall 实现工作流的递归调用
* 【优化】重做ngql格式化
* 【优化】可视化性能优化,
* 【优化】提升表格自动调整列宽的体验
* 【优化】优化关闭程序的体验
* 【新功能】查询结果中，显示总条数与总页数，并支持输入页码进行跳转
* 【新功能】收集匿名操作数据
* 【新功能】本地日志记录新增客户端产生的历史脚本
* 【新功能】打开工作区时自动弹出创建连接的窗口
* 【新功能】打开查询时，自动追加一个查询窗口
* 【新功能】支持多个连接快速折叠
* 【新功能】在边表、节点表的表格页中显示归属的导航层级
* 【新功能】节点表、边表的表格与可视化统一分页器与加载等待
* 【新功能】支持安装包的方式
* 【新功能】新版本更新检查
* 【修复】修复 go 语法规则错误导致的高亮、格式化出错问题
* 【修复】修复 AI 聊天的回车发送导致内容换行的问题
* 【修复】修复查询代码编辑在大量代码的情况下输入，因代码提示而导致界面卡死的问题
* 【修复】修复未填写信息测试连接无法正常显示错误提示的问题
* 【修复】修复切换工作空间时，无法打开该空间历史窗口的问题
* 【修复】修复历史页签不能在启动时准确加载的问题
* 【修复】修复添加连接或切换工作区时，无法及时更新导航树的问题
* 【修复】修复 markdown 在有 TODO 勾选框时，无法渲染的问题
* 【修复】修复因空白字符` `(\u00A0)破坏语法结构的语法高亮问题
* 【界面调整】表格展示移除边框样式
* 【界面调整】修复表格及图可视化字体不一致问题
* 【界面调整】修复浅色主题下，部分字体颜色过浅的问题


# 2.1.0

* 【新功能】支持单表清空
* 【优化】性能提升。当可视化不激活时，关闭其不必要的后台运算避免占用CPU
* 【优化】优化程序退出体验
* 【优化】美化图可视化箭头样式
* 【修复】修复 INT64 超过最大长度半数，变成负数的问题

# 2.0.1

* 【修复】无法手动设置许可证的问题
* 【界面调整】AI 模块的字体与全局保持一致

# 2.0.0

## 山青。

* 【新功能】支持定义代码模板，支持不同作用域，如特定图空间、特定数据源、全局等
* 【新功能】（专业版）集成 AI 大模型，默认 DeepSeek-R1，可配置其他模型
  1. 内置 8 种角色，功能涵盖：创建图谱的工作流、建模、初始化数据、索引创建、脚本调优与修复...
  1. 支持自定义角色、大模型，支持扁平化配置工作流
  1. 支持在查询区中，右键菜单快速调用 AI
  1. 支持快速携带本地表结构（可选，默认不传给 AI）
  1. 支持在提问中，追加数据模式（不上传数据，只上传图数据的表关联，默认不传）
  1. 对话内容支持 svg 渲染
  1. 对话内容、角色、模型api参数本地存储

* 【新功能】（专业版）数据迁移
  1. 图空间数据总览
  1. 选表复制支持部份选取、全部选取，也支持不选（默认为全选）
  1. 数据备份时，同步备份索引并完成索引重建

* 【新功能】后台任务与通知
  1. 优化schema树的加载方式，采用后台任务进行静默加载
  1. 导入功能使用后台任务列表取代加载进度环
  1. 可计算终点的任务，显示具体进度百分比
  1. 对任务的报错信息建立通知机制，通过全局界面右下角查看
  1. 任务列表的详情显示

* 【新功能】授权机制
  1. 在线用户登录时静默安装临时授权码
  1. 支持离线授权
  1. 【操作习惯变更】取消试用功能，改用许可证的方式

* 【操作习惯变更】取消最右侧功能区的【指南】跟【投币】标签页，改为通过文档站查看
* 【操作习惯变更】查询参数区默认最小化
* 【操作习惯变更】将数据源的触发看到 Hosts 信息移到右键【总览】
* 【操作习惯变更】将快速添加查询的按钮移到主操作区的页签中
* 【操作习惯变更】查询区默认不启用执行计划
* 【新功能】主操作区选中页签，导航树同步选中
* 【新功能】在主区中打开的标签，支持在导航树中同步展开节点
* 【新功能】程序启动时，初始化加载已打开过的主页面
* 【新功能】应用的全局信息中，新增设备号
* 【新功能】查询功能根据语法实现代码补全
* 【新功能】添加边时，可查询点并选取作为首尾节点
* 【新功能】树导航中，表对象右键生成 NgBatis 实体类
* 【优化】进一步提升代码提示的体验，根据语法提示
* 【优化】编辑Schema，添加属性时，自动滚动至最低部
* 【优化】表格样式优化，使得30行数据不至于溢出屏幕
* 【修复】修复图可视化中，刷新数据后再触发邻居节点不加载节点的问题
* 【修复】修复边数据过滤不起作用及点过滤时必须有索引的问题
* 【修复】修复图可视化数据面版颜色过浅看不清的问题
* 【修复】使用中文作为默认语言
<!-- * 【修复】尝试修复 macos command+c 无法复制的问题 -->
* 【修复】修复 geo 字段类型的导入问题
* 【修复】修复 geo 类型展示出错问题
* 【修复】修复图空间删除重建同名空间后，标签设置框出错的问题
* 【修复】修复 vid 为 int 时，不设置 id值偏移量无法导入的问题
* 【修复】修复查询区 tab 页过多看不到页签内容的问题
* 【修复】修复查询结果页签过多看不到页签内容的问题
* 【修复】修复边表名称及属性不能使用中文的问题
* 【修复】修复csv导入的类型适配问题
* 【修复】修复同列不同类型的表格值展示问题
* 【修复】修复创建标签弹窗错误的问题
* 【界面调整】优化主操作区按钮越界时的展示效果
* 【界面调整】美化窗口分割线的样式
* 【界面调整】查询区日志页签增加图标
* 【界面调整】部分表单的弹窗增加统一的关闭按钮
* 【界面调整】主操作区最小宽度 40% 的限制改成 100像素


# 1.3.0

## 烛光。

* 【新功能】图可视化支持关闭节点位置移动，并可拖拽节点，手动调整节点位置
* 【新功能】图可视化支持使用字段值作为节点图片展示
* 【新功能】图可视化选中节点后，展示数据节点数据面板
* 【新功能】图可视化支持在触发节点点击时，使用表格分页加载邻居节点
* 【新功能】图可视化布局支持前进后退，控制加载邻居节点
* 【新功能】图可视化布局算法支持调整布局参数
* 【新功能】图可视化支持通过节点类型、边类型控制节点、关系的显示隐藏
* 【新功能】在主界面左下角新增【快速开始】，支持在左下角的入口创建标签、关系、查询
* 【新功能】部分日志功能：用户目录/.star_shadow/logs/日期.log
* 【优化】提高schema导航关闭节点的性能
* 【优化】打开软件时，窗口自动最大化
* 【优化】图可视化浮框数据面板添加 tag 名称
* 【优化】图可视化展示，文字控制在10个字
* **【修复】图可视化中，当节点数量较多时，整个展示区域崩溃成黑屏的问题**
* 【修复】 修复关系名含有运算符，打开出错的问题
* 【修复】 修复设置页在查看许可证后的国际化错误问题
* 【修复】 修复【美化代码】中`!=`被格式化成`=`的问题
* 【修复】 浅色模式下，markdown 文档内容文字颜色显示出错问题
* 【修复】 登录过期，未移除登录状态的问题

# 1.2.0

## 花火。

* 【新功能】新增GitHub发布仓库的入口（摊牌了，就是求star求watch的）
* 【新功能】主窗口页签的关闭功能，在v1.1.0的基础上支持关闭右侧、关闭其他页签
* 【新功能】增加查询时编写脚本的代码补全功能
* 【新功能】编写查询脚本时，代码补全中涉及的函数使用，支持直接跳转数据库产品官网查看文档
* 【新功能】编写查询脚本时，支持右键菜单执行脚本
* 【新功能】编写查询脚本时，支持多行快捷注释`Ctrl+/`
* 【新功能】编写查询脚本时，支持代码查找功能`Ctrl+F`
* 【新功能】编写查询脚本时，...（更多快捷键功能可参考[reqable/re-editor](https://github.com/reqable/re-editor)）
* 【操作习惯变更】查询功能`运行`的快捷键不可用的问题，改为`Alt+R`
* 【操作习惯变更】查询功能`代码美化`的快捷键改为`Alt+L`
* 【操作习惯变更】取消 `Alt+F` 打开文件菜单的快捷键
* 【优化】重构代码编辑器，提高查询脚本编写时整体性能，并引入上述的诸多功能
* 【优化】统一不同组件的字体，使用【阿里普惠体】
* 【优化】导航树节点操作相关后，树结构更新的及时性问题
* 【优化】`删除`按钮的样式问题，使用危险色进行标识
* 【优化】图可视化追加方向性
* 【优化】图可视化，边的起止点位置设置在节点中心，提升视觉体验
* 【修复】图可视化有时出现崩溃的问题
* 【修复】执行计划可视化中的任务节点重叠问题
* 【修复】查询文件重命名，文件名后缀引发的导航树节点丢失问题
* 【修复】schema 导航树元素删除时后，打开的页签未移除问题
* 【修复】修复添加连接使用默认连接的产生的错误问题
* 【修复】查询结果表格，页码与序号出错的问题
* 【修复】desc space 语法未识别的问题

# 1.1.0

## 西陆。

* 【新功能】可视化支持配置节点名称所使用的字段。支持节点颜色配置
* 【新功能】在标签跟关系的数据表格视图中，支持切换至图可视化视图
* 【新功能】数据表格的分页加载模式修改成使用分页按钮的加载方式
* 【新功能】tag、edge数据值在表格内直接修改
* 【新功能】评论支持修改
* 【新功能】查询文件支持在资源管理器中打开
* 【新功能】追加数据源的主区域，支持添加主机
* 【新功能】全平台增加试用功能
* 【新功能】支持快速关闭多个页签
* 【新功能】修复运行脚本时表格数据未及时更新的问题
* 【新功能】反馈模块追加条件过滤功能
* 【新功能】在数据表格中，支持多标签节点单独删除某个标签
* 【优化】图可视化组件美化，兼容浅色主题
* 【优化】修改登录界面，去掉登录视频，压缩安装包大小
* 【优化】优化表格 单元格的选中体验
* 【优化】避免了切换标签页时，页面刷新的问题
* 【优化】刷新界面时，避免二次查询
* 【修复】反馈模块，不是自己的评论时，不显示"..."
* 【修复】修复标签页关闭，存在其他页签未加载完成的进度条
* 【修复】含有换行`\n`的长文本的数据更新问题
* 【修复】部分多标签节点在表格数据下展示出错问题
* 【修复】解决节点多标签时，空值与无标签无法区分的问题
* 【修复】导入不支持时间类型的问题
* 【修复】返回值列名不能为中文的问题
* 【修复】点边schema经过编辑后，表格数据刷新无相应变动的问题
* 【修复】修复字段重命名导致数据丢失问题。（不再支持重命名）
* 【修复】无法打开标签，关系，查询文件的问题
* 【修复】试用功能的勾选隐私政策与服务条款检查
* 【修复】修复查询文件重命名，参数未同步问题
* 【文档】增加关于商业许可的说明

# 1.0.0-beta

## 萌芽。

* 【新功能】图数据展示支持使用 多维表头的表格
* 【新功能】集成图可视化插件，并在编写查询时支持图可视化
* 【新功能】使用导航树的方式便捷操作与管理Schema
* 【新功能】路径格式数据可视化
* 【新功能】子图数据使用表格中的单元格触发弹窗式的内内嵌表格展示
* 【新功能】支持使用 CSV 进行数据导入
* 【新功能】支持使用 CSV 的表头快速创建 Schema
* 【新功能】支持对单表（标签表、关系表）的数据进行增删改查
* 【新功能】支持个人用户使用微信支付的成为我们的个人赞助商
* ...